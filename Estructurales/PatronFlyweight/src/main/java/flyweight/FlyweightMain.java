...
public class FlyweightMain {
    ...
    private static final String[] NombresArtistas = new String[100];
    ...
    private static void CrearListaDinamica() {
        ...
        for (int c = 0; c < NombresListas.length; c++) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                int artist = random.nextInt(NombresArtistas.length);
                playList.addCancion(NombreCanciones[song], NombreCanciones[song]);
            }
            Listas.add(playList);
...
//        }
    ...
    private static void InicializarArreglos() {
        ...
        for (int c = 0; c < NombresArtistas.length; c++) {
            NombresArtistas[c] = "Artista " + (c + 1);
        }
    }
}
